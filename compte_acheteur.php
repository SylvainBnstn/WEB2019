...
try {
    ...
    if (!empty($prenom)&&!empty($nom)&&!empty($pseudo)&&!empty($mdp)&&!empty($email)&&!empty($adresse)&&!empty($ville)&&!empty($code_postal)&&!empty($pays)&&!empty($telephone)&&!empty($numero_carte)&&!empty($nom_proprio)&&!empty($expiration)&&!empty($crypto)&&($mdp2 == $mdp)&&($email2 == $email)) {
        if ($conn)
        {
        	...
        	if (($mailexiste == 0)&&($pseudoexiste == 0)&&(!isset($_SESSION["Default_user"]))||$_SESSION["Default_user"]==false) {
          ...
          if (($mailexiste == 0)&&($pseudoexiste == 0)&&(isset($_SESSION["Default_user"]))||$_SESSION["Default_user"]==true) {
            $id_to_treat=$_SESSION["Id_client"];
...
<html> 
	<head> 
		<meta charset="utf-8"> 
		<title>Création Compte Acheteur - ECE Amazon</title> 
