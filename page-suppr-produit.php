...
if (isset($_SESSION["Connected_acheteur"])&&$_SESSION["Connected_acheteur"]==true) {
	...
	$id_product=$_GET['Id'];
	$achat=$_GET['Ida'];
	$supprproduct=$conn->prepare("SELECT * FROM produit WHERE Id_produit = $id_product");
	$supprproduct->execute();
	$nbproduct=$supprproduct->rowCount();

	if (isset($_POST['Suppr_panier'])) {
		$requete = $conn->prepare("DELETE FROM panier WHERE Id_produit = $id_product AND Id_achat = $achat AND Id_acheteur = $id_acheteur AND Etat_transac=0");
		$requete->execute();
...
<html>
<meta charset="utf-8"> 
<title>Résumé Produit - ECE Amazon</title> 
			...
			if (isset($nbproduct)) {
				if ($nbproduct > 0) {
					...
					foreach($var as $elem=>$value)
					{
						...
						$method="post";
						echo "<br><br><form action='' method='$method'><input class='btn btn-primary btn-lg' type='submit' value='Supprimer du panier' name='Suppr_panier'></form>";
						echo "</div>";
					}
				}
			}
